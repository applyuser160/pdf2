...
pub fn generate_pdf(doc: &Document, path: &Path) -> Result<(), GenerateError> {
    let (pdf_doc, page1, layer1) = PdfDocument::new("Generated PDF", Mm(210.0), Mm(297.0), "Layer 1");
    let current_layer = pdf_doc.get_page(page1).get_layer(layer1);
    let font = pdf_doc.add_builtin_font(printpdf::BuiltinFont::Helvetica).unwrap();

    let full_text = doc.pages.iter()
    ...
    pdf_doc.save(&mut BufWriter::new(File::create(path)?))
